...
public class fightRuneGiants extends Task {
    warriorMain main;
    public fightRuneGiants(warriorMain main) {
        super();
    ...
    public void execute() {
       // Npc cyclops = Npcs.stream().within(Areas.GIANT_FIGHT_AREA).filtered(i -> i.!cyclops.inCombat()).first(); //within(Players.local(), 10).name("Cyclops").first();
        Npc cyclops = Npcs.stream().within(Areas.RUNE_GIANT_FIGHT_AREA).name("Cyclops").filtered(npc
                -> npc.valid()
